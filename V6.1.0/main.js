...
const WW_ACTION = {
  ...
  LAST_WORDS_NEXT  : 'ww_last_words_next',// host advances past the eliminated player's last words
...
const makeWerewolfGame = () => ({
  gameType             : 'werewolf',
  wwPhase              : 'role_reveal',  // role_reveal|night|day_announce|day_discuss|vote|vote_result|last_words|special|end
  wwRound              : 0,
  nightTime            : 30,
  nightTimeLeft        : 30,
  roleConfig           : {},
...
const store = new Store({
  ...
  settings   : {
    ...
    drawConfig: { writeTime: 60, drawTime: 180, guessTime: 45 },
    sketchConfig: { mode: 'time', rounds: 1, turnTime: 120 },
  },
  game       : makeGame(),
});
...
class RoomManager {
  ...
  async joinRoom(rawCode, playerName) {
    ...
    const drawStarted   = gd && gd.gameType === 'draw';  // 鏈條數在開局時就固定，中途加入一律觀戰
    const sketchStarted = gd && gd.gameType === 'sketchrelay'; // 同上：畫布鏈數量開局就固定
    const isSpectator = !!(wwStarted || storyStarted || chaosStarted || madlibStarted || drawStarted || sketchStarted);
...
// ═══════════════════════════════════════════════════════════════════════
// LAYER 9b ─ Drawing Canvas Module（可重用繪圖元件）
//
//   通用畫布元件，不綁定任何特定遊戲。任何遊戲只要：
//     const board = new DrawingBoard(containerEl, { readOnly, onChange });
//   即可掛載一個「完整工具列 + 畫布」，並透過 board.getStrokes() /
//   board.loadStrokes(strokes) 存取／還原筆畫資料（正規化 0–1 座標，
//   與螢幕尺寸無關，Firebase 友善的輕量向量格式）。
// ═══════════════════════════════════════════════════════════════════════
...
class DrawingBoard {
  constructor(container, opts) {
    ...
    this.addOnly   = !!opts.addOnly;   // true = "clear" only wipes this session's own strokes; the eraser is also
                                        // structurally unable to touch locked-in content from previous players
                                        // (see _strokePath/_paintOnto — eraser strokes only ever erase within
                                        // this round's own transparent strokes layer, never the layers beneath it)
    this.onChange  = opts.onChange || null;
    this.strokes   = [];          // vector strokes drawn on top of _baseCanvas since last fill
    this._current  = null;
    this._baseCanvas = null;      // offscreen REF_W×REF_H canvas — "baked" raster (from fills), or null
    this._lockedBase = null;      // addOnly mode: permanently protected inherited content, never touched by undo/clear
    this._preFillSnapshot = null; // one-level undo for the most recent fill
    this._color    = '#2b2b2b';
    this._width    = 0.013;       // normalized stroke width (fraction of min(canvas w,h))
    this._tool     = 'pen';       // 'pen' | 'eraser' | 'bucket'
    this._palette  = ['#2b2b2b','#e63946','#f4a300','#2ec4b6','#3a86ff','#7b61ff','#ff6b9d','#06d6a0','#ffffff'];
    this._build();
  }

  static get REF_W() { return 640; }
  static get REF_H() { return 480; }

  _build() {
    var self = this;
    this.container.innerHTML = '';
    this.container.classList.add('draw-board');

    {
      var toolbar = document.createElement('div');
      toolbar.className = 'draw-toolbar';

      // ── Color row: presets + custom picker ──
      var colorRow = document.createElement('div');
      colorRow.className = 'draw-color-row';
      this._palette.forEach(function(c) {
        var sw = document.createElement('button');
        sw.type = 'button';
        sw.className = 'draw-color-swatch' + (c === self._color ? ' active' : '');
        sw.style.background = c;
        if (c === '#ffffff') sw.classList.add('draw-color-swatch-white');
        sw.title = '畫筆顏色';
        sw.addEventListener('click', function() {
          self._color = c;
          if (self._tool === 'eraser') self._tool = 'pen';
          colorRow.querySelectorAll('.draw-color-swatch').forEach(function(b){ b.classList.remove('active'); });
          sw.classList.add('active');
          var cw = toolbar.querySelector('.draw-color-custom-wrap');
          if (cw) cw.classList.remove('active');
          self._syncToolButtons();
        });
        colorRow.appendChild(sw);
      });

      var customWrap = document.createElement('label');
      customWrap.className = 'draw-color-custom-wrap';
      customWrap.title = '自訂顏色（含色彩選擇器）';
      var customInput = document.createElement('input');
      customInput.type = 'color';
      customInput.className = 'draw-color-input';
      customInput.value = this._color;
      customInput.addEventListener('input', function() {
        self._color = customInput.value;
        if (self._tool === 'eraser') self._tool = 'pen';
        colorRow.querySelectorAll('.draw-color-swatch').forEach(function(b){ b.classList.remove('active'); });
      ...
      // ── Size row: quick slider + live preview dot ──
      var sizeRow = document.createElement('div');
      sizeRow.className = 'draw-size-row';
      var sizeDot = document.createElement('span');
      sizeDot.className = 'draw-size-preview-dot';
      var sizeSlider = document.createElement('input');
      sizeSlider.type = 'range'; sizeSlider.className = 'draw-size-slider';
      sizeSlider.min = '1'; sizeSlider.max = '100';
      var MINW = 0.004, MAXW = 0.05;
      sizeSlider.value = String(Math.round(((this._width - MINW) / (MAXW - MINW)) * 99) + 1);
      var updateDot = function() {
        var px = 4 + (self._width - MINW) / (MAXW - MINW) * 26;
        sizeDot.style.width = px + 'px'; sizeDot.style.height = px + 'px';
      };
      sizeSlider.addEventListener('input', function() {
        self._width = MINW + (parseInt(sizeSlider.value, 10) - 1) / 99 * (MAXW - MINW);
        updateDot();
      });
      updateDot();
      sizeRow.appendChild(sizeDot);
      sizeRow.appendChild(sizeSlider);
      toolbar.appendChild(sizeRow);

      // ── Tool row: brush / shapes / bucket / eraser / undo / clear ──
      var toolRow = document.createElement('div');
      toolRow.className = 'draw-tool-row';

      var brushBtn = document.createElement('button');
      brushBtn.type = 'button'; brushBtn.className = 'draw-tool-brush'; brushBtn.title = '筆刷（畫筆）';
      brushBtn.textContent = '🖌️';
      brushBtn.addEventListener('click', function() { self._tool = 'pen'; self._syncToolButtons(); });
      toolRow.appendChild(brushBtn);

      var divider1 = document.createElement('span');
      divider1.className = 'draw-tool-divider';
      toolRow.appendChild(divider1);

      this._shapeBtns = {};
      var shapeDefs = [
        { key: 'rect',     icon: '▭', title: '矩形' },
        { key: 'ellipse',  icon: '⬭', title: '圓形／橢圓形' },
        { key: 'triangle', icon: '△', title: '三角形' },
        { key: 'line',     icon: '╱', title: '直線' },
      ];
      shapeDefs.forEach(function(def) {
        var b = document.createElement('button');
        b.type = 'button'; b.className = 'draw-tool-shape'; b.title = def.title;
        b.textContent = def.icon;
        b.addEventListener('click', function() {
          self._tool = 'shape'; self._shapeType = def.key;
          self._syncToolButtons();
        });
        toolRow.appendChild(b);
        self._shapeBtns[def.key] = b;
      });

      var divider2 = document.createElement('span');
      divider2.className = 'draw-tool-divider';
      toolRow.appendChild(divider2);

      var ICON_BUCKET = '<svg viewBox="0 0 24 24" width="19" height="19" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linejoin="round" stroke-linecap="round"><path d="M3.8 6.8 Q 1.6 1.8 8.6 2.1"/><path d="M2.3 8.7 L6.6 13.2 L14 8.9 L12.3 2.9"/><ellipse cx="7.3" cy="5.9" rx="5.5" ry="1.85" transform="rotate(-31 7.3 5.9)"/><path d="M13.4 7.2 C15.4 9.3 17 11.4 17.5 13.5"/><path d="M17.5 13.2 c1.6 1.9 2.3 3.1 2.3 4.2 a2.2 2.2 0 0 1 -4.4 0 c0 -1.1 0.7 -2.3 2.1 -4.2 Z" fill="currentColor" stroke="none"/></svg>';
      var ICON_ERASER = '<svg viewBox="0 0 24 24" width="19" height="19" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linejoin="round" stroke-linecap="round"><rect x="4" y="12" width="14" height="7" rx="1.5" transform="rotate(-28 11 15.5)"/><line x1="7" y1="20" x2="21" y2="20"/></svg>';

      var bucketBtn = document.createElement('button');
      bucketBtn.type = 'button'; bucketBtn.className = 'draw-tool-bucket'; bucketBtn.title = '油漆桶（區域填色）';
      bucketBtn.innerHTML = ICON_BUCKET;
      bucketBtn.addEventListener('click', function() { self._tool = 'bucket'; self._syncToolButtons(); });
      toolRow.appendChild(bucketBtn);
      this._bucketBtn = bucketBtn;

      var eraserBtn = document.createElement('button');
      eraserBtn.type = 'button'; eraserBtn.className = 'draw-tool-eraser';
      eraserBtn.title = this.addOnly ? '橡皮擦（只會擦到你這輪自己加入的筆跡）' : '橡皮擦';
      eraserBtn.innerHTML = ICON_ERASER;
      eraserBtn.addEventListener('click', function() { self._tool = 'eraser'; self._syncToolButtons(); });
      toolRow.appendChild(eraserBtn);
      this._eraserBtn = eraserBtn;


      var undoBtn = document.createElement('button');
      undoBtn.type = 'button'; undoBtn.className = 'draw-tool-undo';
      undoBtn.title = this.addOnly ? '復原上一筆（只會動到你這輪自己畫的部分）' : '復原上一筆';
      undoBtn.textContent = '↩️';
      undoBtn.addEventListener('click', function() { self.undo(); });
      toolRow.appendChild(undoBtn);

      var clearBtn = document.createElement('button');
      clearBtn.type = 'button'; clearBtn.className = 'draw-tool-clear';
      clearBtn.title = this.addOnly ? '清空我這輪新增的內容（不會動到別人畫的部分）' : '清空畫布';
      clearBtn.textContent = '🗑️';
      clearBtn.addEventListener('click', function() {
        if (self.isBlank()) return;
        var msg = self.addOnly ? '確定清空你這一輪新增的內容？（不會動到別人畫的部分）' : '確定清空整張畫布？';
        if (confirm(msg)) self.clear();
      });
      ...
      this._brushBtn = brushBtn;
      this._shapeType = this._shapeType || 'rect';
      this._syncToolButtons();
      toolbar.classList.toggle('hidden', this.readOnly);
    }
    ...
    if (window.ResizeObserver) {
      ...
      this._ro.observe(canvasWrap);
    }

    if (!this.readOnly) this._bindPointer();
  }

  _syncToolButtons() {
    if (this._brushBtn)  this._brushBtn.classList.toggle('active', this._tool === 'pen');
    if (this._bucketBtn) this._bucketBtn.classList.toggle('active', this._tool === 'bucket');
    if (this._eraserBtn) this._eraserBtn.classList.toggle('active', this._tool === 'eraser');
    if (this._shapeBtns) {
      var self = this;
      Object.keys(this._shapeBtns).forEach(function(k) {
        self._shapeBtns[k].classList.toggle('active', self._tool === 'shape' && self._shapeType === k);
      });
    }
  }
  ...
  _resize() {
    ...
    var h = wrap.clientHeight || Math.round(w * 0.72);
  ...
  _pointFromClient(clientX, clientY) {
    ...
    return {
      x: Utils.clamp((clientX - rect.left) / rect.width,  0, 1),
      y: Utils.clamp((clientY - rect.top)  / rect.height, 0, 1),
    };
  }

  // Extracts the point for a specific tracked touch identifier (or the first
  // touch/mouse point when trackedId is null). Returns null if that finger
  // isn't present in this event — used to ignore any OTHER finger that
  // touches the canvas while one is already drawing.
  _extractPoint(e, trackedId) {
    if (!e.touches) return { clientX: e.clientX, clientY: e.clientY };
    if (trackedId == null) {
      return e.touches[0] ? { clientX: e.touches[0].clientX, clientY: e.touches[0].clientY } : null;
    }
    for (var i = 0; i < e.touches.length; i++) {
      if (e.touches[i].identifier === trackedId) {
  ...
  _bindPointer() {
    var self = this, drawing = false, touchId = null;

    function start(e) {
      if (drawing) return; // a stroke is already in progress — ignore any extra finger
      var pt = self._extractPoint(e, null);
      if (!pt) return;
      e.preventDefault();
      if (e.touches && e.touches.length) touchId = e.touches[0].identifier;
      var p = self._pointFromClient(pt.clientX, pt.clientY);
      if (self._tool === 'bucket') { self._performFill(p.x, p.y); touchId = null; return; }
      drawing = true;
      if (self._tool === 'shape') {
        self._current = { tool: 'shape', shapeType: self._shapeType, color: self._color, width: self._width, points: [p, p] };
      } else {
        self._current = { tool: self._tool, color: self._color, width: self._width, points: [p] };
      }
      self._redraw(self._current);
    }
    function move(e) {
      if (!drawing) return;
      var pt = self._extractPoint(e, touchId);
      if (!pt) return; // the tracked finger isn't part of this event — some other finger moved
      e.preventDefault();
      var p = self._pointFromClient(pt.clientX, pt.clientY);
      if (self._current.tool === 'shape') {
        self._current.points[1] = p; // only the end corner moves — start corner stays fixed
      } else {
    ...
    function end(e) {
      if (!drawing) return;
      if (e.touches) {
        // if our tracked finger is still down, this end event belongs to a DIFFERENT finger — ignore
        for (var i = 0; i < e.touches.length; i++) if (e.touches[i].identifier === touchId) return;
      }
      drawing = false; touchId = null;
      if (self._current && self._current.points.length) {
    ...
    this._unbindPointer = function() {
      self.canvas.removeEventListener('mousedown', start);
      self.canvas.removeEventListener('mousemove', move);
      window.removeEventListener('mouseup', end);
      self.canvas.removeEventListener('touchstart', start);
      self.canvas.removeEventListener('touchmove', move);
      self.canvas.removeEventListener('touchend', end);
      self.canvas.removeEventListener('touchcancel', end);
    };
  }

  _strokePath(ctx, stroke, w, h) {
    var min = Math.min(w, h);
    ctx.lineWidth   = Math.max(1, stroke.width * min);
    ctx.lineCap     = 'round';
    ctx.lineJoin    = 'round';
    if (stroke.tool === 'eraser') {
      // Punch a hole in whatever this ctx itself has drawn so far. When called
      // against the isolated strokes overlay in _paintOnto, that means an
      // eraser can only ever remove ink that THIS round's own strokes put
      // there — it structurally cannot reach the locked base / baked fills
      // the overlay later gets composited on top of.
      ctx.globalCompositeOperation = 'destination-out';
      ctx.strokeStyle = '#000000'; // colour is irrelevant for destination-out — only coverage matters
    } else {
    ...
    if (stroke.tool === 'shape') {
      ...
      var x0 = p0.x * w, y0 = p0.y * h, x1 = p1.x * w, y1 = p1.y * h;
  ...
  // Draws current committed state (baked base + vector strokes) onto any ctx at size w×h
  _paintOnto(ctx, w, h, includeLive) {
    ctx.save();
    ctx.globalCompositeOperation = 'source-over';
    ctx.fillStyle = '#ffffff';
    ctx.fillRect(0, 0, w, h);
    if (this._lockedBase) ctx.drawImage(this._lockedBase, 0, 0, w, h);
    if (this._baseCanvas) ctx.drawImage(this._baseCanvas, 0, 0, w, h);
    // This round's own strokes are rendered onto an isolated transparent layer
    // first (see _strokePath) so that eraser strokes can only erase ink that
    // THIS layer itself drew — never the locked/base layers above.
    var overlay = document.createElement('canvas');
    overlay.width = w; overlay.height = h;
    var octx = overlay.getContext('2d');
    var self = this;
    this.strokes.forEach(function(s) { self._strokePath(octx, s, w, h); });
    if (includeLive && this._current) this._strokePath(octx, this._current, w, h);
  ...
  _redraw(liveStroke) {
    if (!this.ctx) return;
    this._paintOnto(this.ctx, this.canvas.width, this.canvas.height, !!liveStroke);
  }
  ...
  _performFill(xNorm, yNorm) {
    var RW = DrawingBoard.REF_W, RH = DrawingBoard.REF_H;
    var scratch = document.createElement('canvas');
    scratch.width = RW; scratch.height = RH;
    var sctx = scratch.getContext('2d');
    this._paintOnto(sctx, RW, RH, false);

    var px = Math.min(RW - 1, Math.max(0, Math.round(xNorm * RW)));
    var py = Math.min(RH - 1, Math.max(0, Math.round(yNorm * RH)));

    var imgData = sctx.getImageData(0, 0, RW, RH);
    var rgb = this._hexToRgb(this._color);
    var changed = this._floodFillImageData(imgData, px, py, rgb, 60);
    if (!changed) return; // clicked area already (near) target color — no-op

    sctx.putImageData(imgData, 0, 0);

    this._preFillSnapshot = { baseCanvas: this._baseCanvas, strokes: this.strokes.slice() };
    this._baseCanvas = scratch;
    this.strokes = [];
    this._redraw();
    this._emitChange();
  }

  _floodFillImageData(imageData, startX, startY, fillRgb, tolerance) {
    var data = imageData.data, w = imageData.width, h = imageData.height;
    var idx0 = (startY * w + startX) * 4;
    var tR = data[idx0], tG = data[idx0+1], tB = data[idx0+2], tA = data[idx0+3];
    var fR = fillRgb.r, fG = fillRgb.g, fB = fillRgb.b;
    if (Math.abs(tR-fR) <= tolerance && Math.abs(tG-fG) <= tolerance && Math.abs(tB-fB) <= tolerance && tA > 200) {
      return false; // already essentially this color
    }
    ...
    while (stack.length) {
      ...
      var di = vi * 4;
      var dr = data[di], dg = data[di+1], db = data[di+2], da = data[di+3];
      if (Math.abs(dr-tR) > tolerance || Math.abs(dg-tG) > tolerance || Math.abs(db-tB) > tolerance || Math.abs(da-tA) > tolerance) continue;
      visited[vi] = 1;
  ...
  // ── Undo / Clear ──────────────────────────────────────
  undo() {
    if (this.strokes.length > 0) { this.strokes.pop(); this._redraw(); this._emitChange(); return; }
    if (this._preFillSnapshot) {
      this._baseCanvas = this._preFillSnapshot.baseCanvas;
      this.strokes = this._preFillSnapshot.strokes;
      this._preFillSnapshot = null;
      this._redraw(); this._emitChange();
    }
  }

  clear() {
    this.strokes = []; this._baseCanvas = null; this._preFillSnapshot = null;
    this._redraw(); this._emitChange();
  }

  isBlank() { return !this._baseCanvas && this.strokes.length === 0; }

  // ── Serialization ─────────────────────────────────────
  // Returns { base: dataURLOrNull, strokes: [...] } — the vector strokes are
  // whatever sits on top of the last baked fill (or everything, if no fill was used).
  // In addOnly mode with locked-in prior content, everything (locked + editable
  // layers) gets flattened together, since the next player needs the full picture.
  getStrokes() {
    if (this.addOnly && this._lockedBase) {
      var RW = DrawingBoard.REF_W, RH = DrawingBoard.REF_H;
      var c = document.createElement('canvas');
      c.width = RW; c.height = RH;
      this._paintOnto(c.getContext('2d'), RW, RH, false);
      return { base: c.toDataURL('image/png'), strokes: [] };
    }
    return {
      base: this._baseCanvas ? this._baseCanvas.toDataURL('image/png') : null,
      strokes: Utils.deepClone(this.strokes),
    };
  }

  // Accepts either the new { base, strokes } shape or a legacy plain strokes array.
  // In addOnly mode, whatever is loaded gets baked into a permanently protected
  // "locked base" layer — undo/clear/fill on this session can never reach into it,
  // no matter which tools get used. The editable layer always starts fresh.
  loadStrokes(data) {
    var base, strokes;
    if (Array.isArray(data)) { base = null; strokes = data; }
    else { data = data || {}; base = data.base || null; strokes = data.strokes || []; }

    if (this.addOnly) {
      this.strokes = []; this._baseCanvas = null; this._preFillSnapshot = null;
      this._bakeIntoLockedBase(base, strokes);
      return;
    }

    this.strokes = Utils.deepClone(strokes);
    this._preFillSnapshot = null;
    if (!base) { this._baseCanvas = null; this._redraw(); return; }
    var self = this, img = new Image();
    img.onload = function() {
      var c = document.createElement('canvas');
      c.width = DrawingBoard.REF_W; c.height = DrawingBoard.REF_H;
  ...
  // Flattens a base image + stroke array into an immutable locked-base canvas.
  _bakeIntoLockedBase(base, strokes) {
    var self = this;
    var RW = DrawingBoard.REF_W, RH = DrawingBoard.REF_H;
    function finish(baseImg) {
      var c = document.createElement('canvas');
      c.width = RW; c.height = RH;
      var ctx = c.getContext('2d');
      ctx.fillStyle = '#ffffff'; ctx.fillRect(0, 0, RW, RH);
      if (baseImg) ctx.drawImage(baseImg, 0, 0, RW, RH);
      (strokes || []).forEach(function(s) { self._strokePath(ctx, s, RW, RH); });
      self._lockedBase = c;
      self._redraw();
    }
    if (base) {
  ...
  setReadOnly(v) {
    v = !!v;
    if (v === this.readOnly) return;
    this.readOnly = v;
    if (this._toolbar) this._toolbar.classList.toggle('hidden', this.readOnly);
    if (this.readOnly) {
      if (this._unbindPointer) { this._unbindPointer(); this._unbindPointer = null; }
    } else {
      if (!this._unbindPointer) this._bindPointer();
    }
  }

  _emitChange() { if (this.onChange) this.onChange(this.getStrokes()); }

  destroy() {
    if (this._unbindPointer) this._unbindPointer();
...
class WerewolfEngine {
  ...
  _dispatch(a) {
    ...
    else if (t === WW_ACTION.LAST_WORDS_NEXT)  this._afterLastWords();
  ...
  _resolveVote() {
    ...
    if (role === 'bomber') {
      ...
      this.broadcast();
      setTimeout(() => this._enterLastWords(), 3500);
      return;
    }
  ...
  // ── Last Words (放逐/引爆結果揭曉後，讓被出局玩家留下遺言，由房主按鈕推進) ──
  // Called once the vote_result reveal has played out and someone was actually
  // eliminated. If the game is already won, skip straight to the end screen —
  // otherwise hold on 'last_words' until the host advances.
  _enterLastWords() {
    if (this._checkWin()) return;
    store.patchGame({ wwPhase: 'last_words' });
    this.broadcast();
  }

  _afterLastWords() {
    const g = store.get().game;
    if (g.wwPhase !== 'last_words') return;
    const role = g.roles[g.voteEliminated];
    if (role === 'hunter') {
      // Same private-decide flow used for wolf-killed hunters — wwPhase stays
      // 'last_words' for everyone else; only the hunter sees the decide panel.
      store.patchGame({ hunterDecidePending: { pid: g.voteEliminated, cause: 'vote' } });
      this.broadcast();
    } else {
      this._startNight();
    }
...
// ML_TEMPLATES is now defined in the separate madlib-templates.js file
// (loaded via <script> tag before this file), so it can be shared with
// admin.html without duplicating the ~800-line template bank.
...
// ═══════════════════════════════════════════════════════════════════════
// LAYER 10e ─ Draw Relay Engine (傳畫接龍)
//
//   N 位玩家 = N 條鏈，全員每回合同時行動，完成後鏈條固定往下家輪傳，
//   總共跑 N 回合（不繞回原主）。回合類型：
//     第 1 回合          → write（起始題目）
//     第 r 回合（r≥2）   → r 為偶數 → draw；r 為奇數 → guess
//   畫作以向量筆畫（DrawingBoard 格式）儲存，文字直接存字串。
// ═══════════════════════════════════════════════════════════════════════

const DRAW_ACTION = {
  SUBMIT   : 'draw_submit',
  UNSUBMIT : 'draw_unsubmit',
  REVEAL_NEXT  : 'draw_reveal_next',
  REACT        : 'draw_react',
  RETURN_LOBBY : 'draw_return_lobby',
};

const makeDrawGame = () => ({
  gameType    : 'draw',
  drawPhase   : 'write',     // write | draw | guess | reveal
  drawRound   : 1,
  totalRounds : 0,           // = N，於 startGame 時設定
  writeTime   : 60,
  drawTime    : 180,
  guessTime   : 45,
  timeLeft    : 0,
  pids        : [],          // 固定玩家順序（整場遊戲不變）
  chains      : [],          // [{ starterPid, entries:[{type,authorPid,text?,strokes?}] }]
  assignment  : {},          // { pid: chainIndex } — 本回合每人負責哪條鏈
  submissions : {},          // { pid: content } — 本回合提交內容（文字或筆畫陣列）
  gameStartTs : 0,
  revealChainIndex: 0,
  revealEntryIndex : -1,
  revealDone       : false,
...
class DrawRelayEngine {
  constructor() {
    this._timer = null;
    bus.on(EVT.ACTION_RECEIVED, ({ action }) => {
      if (!store.get().isHost) return;
      if ((store.get().game || {}).gameType !== 'draw') return;
      this._dispatch(action);
    });
  }
  ...
  _dispatch(a) {
    const t = a.type, pid = a.playerId;
    if      (t === DRAW_ACTION.SUBMIT)       this._submit(pid, a.content);
    else if (t === DRAW_ACTION.UNSUBMIT)     this._unsubmit(pid);
    else if (t === DRAW_ACTION.REVEAL_NEXT)  this._revealNext();
    else if (t === DRAW_ACTION.REACT)        this._react(pid, a.emoji);
    else if (t === DRAW_ACTION.RETURN_LOBBY) { if (pid === store.get().hostId) this._returnLobby(); }
  }

  startGame(cfg) {
    const { players } = store.get();
    const pids = Utils.shuffle(Object.keys(players).filter(pid => !players[pid].isSpectator));
    const N = pids.length;
    const chains = pids.map(pid => ({ starterPid: pid, entries: [] }));
    const writeTime = cfg.writeTime || 60;
    store.replaceGame(Object.assign(makeDrawGame(), {
      writeTime, drawTime: cfg.drawTime || 180, guessTime: cfg.guessTime || 45,
      totalRounds: N, drawRound: 1, drawPhase: 'write',
      pids, chains, assignment: this._computeAssignment(pids, 1),
      submissions: {}, timeLeft: writeTime,
      gameStartTs: Date.now(),
    }));
    this.broadcast();
    this._startTimer(writeTime, () => this._resolveRound());
  }

  _computeAssignment(pids, round) {
    const N = pids.length, a = {};
    pids.forEach((pid, i) => {
      // 玩家 i 在第 round 回合負責的鏈 c，滿足鏈 c 的接手者序列為 pids[c], pids[c+1], pids[c+2]...
      const c = ((i - (round - 1)) % N + N) % N;
      a[pid] = c;
    });
    return a;
  }

  _roundType(round) { return round === 1 ? 'write' : (round % 2 === 0 ? 'draw' : 'guess'); }

  _roundTime(g, type) {
    return type === 'draw' ? g.drawTime : (type === 'guess' ? g.guessTime : g.writeTime);
  }

  _submit(pid, content) {
    const g = store.get().game;
    if (!['write','draw','guess'].includes(g.drawPhase)) return;
    if (this._activePids().indexOf(pid) === -1) return;
    store.patchGame({ submissions: Object.assign({}, g.submissions, { [pid]: content }) });
    this.broadcast();
    const fresh = store.get().game;
    if (this._activePids().every(p => fresh.submissions[p] !== undefined)) { this.stopTimer(); this._resolveRound(); }
  }

  _unsubmit(pid) {
    const g = store.get().game;
    if (!['write','draw','guess'].includes(g.drawPhase)) return;
    const submissions = Object.assign({}, g.submissions); delete submissions[pid];
    store.patchGame({ submissions }); this.broadcast();
  }

  _resolveRound() {
    const g = store.get().game;
    const pids = this._activePids();
    const type = g.drawPhase;
    const submissions = Object.assign({}, g.submissions);
    pids.forEach(p => {
      if (submissions[p] === undefined) submissions[p] = (type === 'draw') ? { base: null, strokes: [] } : '（時間到，未輸入）';
    });

    const chains = Utils.deepClone(g.chains);
    pids.forEach(p => {
      const chainIdx = g.assignment[p];
      if (chainIdx === undefined || !chains[chainIdx]) return;
      const entry = { type, authorPid: p };
      if (type === 'draw') entry.strokes = submissions[p]; else entry.text = submissions[p];
      chains[chainIdx].entries.push(entry);
    });

    const nextRound = g.drawRound + 1;
    if (nextRound > g.totalRounds) {
      store.patchGame({
        chains, submissions: {}, drawPhase: 'reveal',
        revealChainIndex: 0, revealEntryIndex: -1, revealDone: false, reactions: {},
      });
      this.broadcast();
      return;
    }
    const nextType = this._roundType(nextRound);
    const nextAssignment = this._computeAssignment(g.pids, nextRound);
    const nextTime = this._roundTime(g, nextType);
    store.patchGame({
      chains, drawRound: nextRound, drawPhase: nextType,
      assignment: nextAssignment, submissions: {}, timeLeft: nextTime,
    });
    this.broadcast();
    this._startTimer(nextTime, () => this._resolveRound());
  }

  // ── Reveal: 逐鏈、逐格播放（類似規則混亂的逐張揭牌）──────
  _revealNext() {
    const g = store.get().game;
    if (g.drawPhase !== 'reveal' || g.revealDone) return;
    const chains = g.chains;
    let ci = g.revealChainIndex, ei = g.revealEntryIndex + 1;
    if (ei >= (chains[ci] || { entries: [] }).entries.length) {
      ci += 1; ei = 0;
      if (ci >= chains.length) {
        store.patchGame({ revealDone: true });
        this.broadcast();
        return;
      }
    }
    store.patchGame({ revealChainIndex: ci, revealEntryIndex: ei });
    this.broadcast();
  }

  _react(pid, emoji) {
    const g = store.get().game;
    if (g.drawPhase !== 'reveal') return;
    const allowed = ['😂','🔥','💀','👏','😮','🤣','👎','💯'];
    if (!allowed.includes(emoji)) return;
    const cur0 = (g.reactions || {})[pid];
    const doFire = () => {
...
const drawEngine = new DrawRelayEngine();

// ═══════════════════════════════════════════════════════════════════════
// LAYER 10f ─ Sketch Relay Engine (繪畫接龍)
//
//   跟故事接龍完全同構，只是把「文字段落」換成「畫面快照」：
//   N 位玩家 = N 條畫布鏈，每輪全員同時在自己被分配到的畫布上接續作畫
//   （看得到前面所有人畫的內容，直接畫在上面），送出後畫布鏈輪轉一位，
//   重複 rounds 輪。揭曉時逐鏈、逐輪播放畫面演變過程。
// ═══════════════════════════════════════════════════════════════════════

const SKETCH_ACTION = {
  LOCK        : 'sketch_lock',
  UNLOCK      : 'sketch_unlock',
  REVEAL_NEXT : 'sketch_reveal_next',
  REACT       : 'sketch_react',
  RETURN_LOBBY: 'sketch_return_lobby',
};

const makeSketchGame = () => ({
  gameType     : 'sketchrelay',
  phase        : 'drawing',   // 'drawing' | 'reveal'
  currentTurn  : 1,
  totalTurns   : 0,
  turnsPerRound: 0,
  totalRounds  : 2,
  mode         : 'round',     // 'round' | 'time'
  turnTime     : 120,
  timeLeft     : 0,
  gameStartTs  : 0,
  assignments  : {},          // { pid: chainIndex } — 本輪每人負責哪條畫布鏈
  canvases     : [],          // [ [ {authorId, authorName, snapshot:{base,strokes}} ... ], ... ]
  locked       : {},
  submissions  : {},          // { pid: {base,strokes} } — 本輪 WIP 快照
  revealChainIndex: 0,
  revealEntryIndex : -1,
  revealDone       : false,
  reactions        : {},
...
class SketchRelayEngine {
  constructor() {
    this._timer = null;
    this._advTimer = null;
    this._timerAdvTimer = null;
    bus.on(EVT.ACTION_RECEIVED, ({ action }) => {
      if (!store.get().isHost) return;
      if ((store.get().game || {}).gameType !== 'sketchrelay') return;
      this._dispatch(action);
    });
  }
  ...
  _dispatch(a) {
    const t = a.type, pid = a.playerId;
    if      (t === SKETCH_ACTION.LOCK)         this._lock(pid, a.content);
    else if (t === SKETCH_ACTION.UNLOCK)       this._unlock(pid);
    else if (t === SKETCH_ACTION.REVEAL_NEXT)  this._revealNext();
    else if (t === SKETCH_ACTION.REACT)        this._react(pid, a.emoji);
    else if (t === SKETCH_ACTION.RETURN_LOBBY) { if (pid === store.get().hostId) this._returnLobby(); }
  }

  startGame(settings) {
    const { players } = store.get();
    const activePids = Object.keys(players).filter(id => !players[id].isSpectator);
    const N = activePids.length;
    const shuffled = Utils.shuffle(activePids.slice());
    const canvases = shuffled.map(() => []);
    const assignments = {};
    shuffled.forEach((pid, i) => { assignments[pid] = i; });

    store.replaceGame(Object.assign(makeSketchGame(), {
      currentTurn: 1, totalTurns: N * settings.rounds, turnsPerRound: N, totalRounds: settings.rounds,
      mode: settings.mode, turnTime: settings.turnTime,
      timeLeft: settings.mode === 'time' ? settings.turnTime : 0,
      gameStartTs: Date.now(),
      assignments, canvases,
    }));
    this.broadcast();
    if (settings.mode === 'time') this._startTimer(settings.turnTime);
  }
  ...
  _lock(pid, content) {
    ...
    if (!content) return;
    store.patchGame({
      submissions: Object.assign({}, g.submissions, { [pid]: content }),
      locked     : Object.assign({}, g.locked, { [pid]: true }),
    });
    this.broadcast();
  ...
  _unlock(pid) {
    const g = store.get().game;
    if (g.phase !== 'drawing') return;
    const locked = Object.assign({}, g.locked); delete locked[pid];
    store.patchGame({ locked }); this.broadcast();
  }

  _checkAllLocked() {
    const g = store.get().game;
    if (g.phase !== 'drawing') return;
    const pids = Object.keys(g.assignments || {});
    if (pids.length > 0 && pids.every(pid => g.locked[pid])) {
      clearTimeout(this._advTimer);
      this._advTimer = setTimeout(() => this._advance(), 900);
    }
  }
  ...
  _advance() {
    ...
    const canvases = Utils.deepClone(g.canvases);

    const assignments = g.assignments || {};
    const submissions = Object.assign({}, g.submissions);
    const locked = Object.assign({}, g.locked);
    const { players } = store.get();
    Object.keys(assignments).forEach(pid => {
      if (!locked[pid]) { submissions[pid] = submissions[pid] || { base: null, strokes: [] }; locked[pid] = true; }
    });

    Object.keys(assignments).forEach(pid => {
      const snapshot = submissions[pid];
      const idx = assignments[pid];
      if (idx !== undefined && snapshot) {
        canvases[idx].push({ authorId: pid, authorName: (players[pid] || {}).name || '???', snapshot });
      }
    });

    const nextTurn = g.currentTurn + 1;
    if (nextTurn > g.totalTurns) {
      store.patchGame({
        phase: 'reveal', canvases, locked: {}, submissions: {},
        revealChainIndex: 0, revealEntryIndex: -1, revealDone: false, reactions: {},
      });
      this.broadcast();
      return;
    ...
    const chainCount = canvases.length;
    const newAssignments = {};
    Object.keys(g.assignments).forEach(pid => { newAssignments[pid] = (g.assignments[pid] + 1) % chainCount; });

    store.patchGame({
      phase: 'drawing', currentTurn: nextTurn, canvases,
      assignments: newAssignments, locked: {}, submissions: {},
      timeLeft: g.mode === 'time' ? g.turnTime : 0,
    });
    this.broadcast();
    if (g.mode === 'time') this._startTimer(g.turnTime);
  }

  // ── Reveal: 逐鏈、逐輪播放（跟傳畫接龍同一套模式）──────
  _revealNext() {
    const g = store.get().game;
    if (g.phase !== 'reveal' || g.revealDone) return;
    const canvases = g.canvases;
    let ci = g.revealChainIndex, ei = g.revealEntryIndex + 1;
    if (ei >= (canvases[ci] || []).length) {
      ci += 1; ei = 0;
      if (ci >= canvases.length) { store.patchGame({ revealDone: true }); this.broadcast(); return; }
    }
    store.patchGame({ revealChainIndex: ci, revealEntryIndex: ei });
    this.broadcast();
  }
  ...
  _startTimer(sec) {
    ...
    this._timer = setInterval(() => {
      ...
      if (t <= 0) {
        this.stopTimer();
        clearTimeout(this._timerAdvTimer);
        this._timerAdvTimer = setTimeout(() => this._advance(), 3000);
      }
    }, 1000);
  }
...
const sketchEngine = new SketchRelayEngine();
...
class UIController {
  constructor() {
    ...
    bus.on(EVT.GAME_STATE_UPDATED, ({ state }) => {
      ...
      if (state.gameType === 'draw') {
        if (this._screen !== 'draw-game') {
          document.querySelectorAll('.screen').forEach(el => {
            el.classList.add('hidden'); el.classList.remove('active');
          });
          const elD = document.getElementById('screen-draw-game');
          if (elD) { elD.classList.remove('hidden'); elD.classList.add('active'); }
          this._screen = 'draw-game';
        }
        this._renderDraw(Object.assign({}, store.get(), { game: state }));
        return;
      }

      if (state.gameType === 'sketchrelay') {
        if (this._screen !== 'sketch-game') {
          document.querySelectorAll('.screen').forEach(el => {
            el.classList.add('hidden'); el.classList.remove('active');
          });
          const elS = document.getElementById('screen-sketch-game');
          if (elS) { elS.classList.remove('hidden'); elS.classList.add('active'); }
          this._screen = 'sketch-game';
        }
        this._renderSketch(Object.assign({}, store.get(), { game: state }));
        return;
      }

      // WW / chaos / madlib / draw / sketch game ended / returned to lobby
      if (this._screen === 'ww-game' || this._screen === 'chaos-game' || this._screen === 'madlib' || this._screen === 'draw-game' || this._screen === 'sketch-game') {
  ...
  init() {
    ...
    this._bindDrawGame();
    this._bindSketchGame();
    this._bindUnifiedChat();
    this._bindShareInvite();
  ...
  _sync(s) {
    ...
    if (this._screen === 'draw-game') {
      const g = s.game;
      if (!g || g.gameType !== 'draw') return;
      if (s.isHost) this._renderDraw(s);
    }
    if (this._screen === 'sketch-game') {
      const g = s.game;
      if (!g || g.gameType !== 'sketchrelay') return;
      if (s.isHost) this._renderSketch(s);
    }
  }
  ...
  _renderRoomControls(s) {
    ...
    if (s.isHost) {
      ...
      this._show('draw-settings',   gameType === 'draw');
      this._show('sketch-settings', gameType === 'sketchrelay');

      if (gameType === 'story') {
        const storyTimerEl = document.getElementById('story-timer-enabled');
        if (storyTimerEl) storyTimerEl.checked = mode === 'time';
        this._show('time-setting', mode === 'time');
      }
      if (gameType === 'sketchrelay') {
        const skc = (s.settings || {}).sketchConfig || { mode: 'time', rounds: 1, turnTime: 120 };
        const timerEl = document.getElementById('sketch-timer-enabled');
        if (timerEl) timerEl.checked = skc.mode === 'time';
        this._show('sketch-time-setting', skc.mode === 'time');
        const roundsEl = document.getElementById('sketch-rounds');
        if (roundsEl && document.activeElement !== roundsEl) roundsEl.value = skc.rounds || 1;
        const turnTimeEl = document.getElementById('sketch-turn-time');
        if (turnTimeEl && document.activeElement !== turnTimeEl) turnTimeEl.value = skc.turnTime || 120;
      }
  ...
  _renderSpectatorLobby(settings) {
    ...
    var lines = '';
    if (gameType === 'story') {
      var mode = settings.mode || 'time';
      var rounds = settings.rounds || 1;
      lines = '<div class="spec-lobby-row"><span>📖 故事接龍</span></div>' +
        '<div class="spec-lobby-row"><span class="slr-label">倒數計時</span><span>' + (mode==='time'?'已啟用':'已關閉') + '</span></div>' +
      ...
      var dc = settings.drawConfig || {};
      lines = '<div class="spec-lobby-row"><span>🎨 傳畫接龍</span></div>' +
        '<div class="spec-lobby-row"><span class="slr-label">出題時限</span><span>' + (dc.writeTime||60) + ' 秒</span></div>' +
        '<div class="spec-lobby-row"><span class="slr-label">繪圖時限</span><span>' + (dc.drawTime||180) + ' 秒</span></div>' +
        '<div class="spec-lobby-row"><span class="slr-label">猜測時限</span><span>' + (dc.guessTime||45) + ' 秒</span></div>';
    } else if (gameType === 'sketchrelay') {
      var skc = settings.sketchConfig || {};
      lines = '<div class="spec-lobby-row"><span>🖼️ 繪畫接龍</span></div>' +
        '<div class="spec-lobby-row"><span class="slr-label">倒數計時</span><span>' + (skc.mode==='time'?'已啟用':'已關閉') + '</span></div>' +
        '<div class="spec-lobby-row"><span class="slr-label">回合數</span><span>' + (skc.rounds||1) + '</span></div>' +
        (skc.mode==='time' ? '<div class="spec-lobby-row"><span class="slr-label">每輪秒數</span><span>' + (skc.turnTime||120) + ' 秒</span></div>' : '');
    } else {
  ...
  _renderSettingsPreview(settings) {
    ...
    if (gameType === 'story') {
      ...
      cont.innerHTML =
        ...
        '<div class="preview-row"><span class="preview-label">倒數計時</span><span class="preview-val">' + (mode==='time'?'已啟用（'+turnTime+' 秒/輪）':'已關閉（全員鎖定才換輪）') + '</span></div>' +
      ...
      const dc = settings.drawConfig || {};
      cont.innerHTML =
        '<div class="preview-row"><span class="preview-label">遊戲模式</span><span class="preview-val">🎨 傳畫接龍</span></div>' +
        '<div class="preview-row"><span class="preview-label">出題時限</span><span class="preview-val">' + (dc.writeTime||60) + ' 秒</span></div>' +
        '<div class="preview-row"><span class="preview-label">繪圖時限</span><span class="preview-val">' + (dc.drawTime||180) + ' 秒</span></div>' +
        '<div class="preview-row"><span class="preview-label">猜測時限</span><span class="preview-val">' + (dc.guessTime||45) + ' 秒</span></div>';
    } else if (gameType === 'sketchrelay') {
      const skc = settings.sketchConfig || {};
      cont.innerHTML =
        '<div class="preview-row"><span class="preview-label">遊戲模式</span><span class="preview-val">🖼️ 繪畫接龍</span></div>' +
        '<div class="preview-row"><span class="preview-label">倒數計時</span><span class="preview-val">' + (skc.mode==='time'?'已啟用':'已關閉') + '</span></div>' +
        '<div class="preview-row"><span class="preview-label">回合數</span><span class="preview-val">' + (skc.rounds||1) + '</span></div>' +
        (skc.mode==='time' ? '<div class="preview-row"><span class="preview-label">每輪秒數</span><span class="preview-val">' + (skc.turnTime||120) + ' 秒</span></div>' : '');
    } else {
  ...
  _bindRoom() {
    ...
    this._on('btn-start-game', 'click', function() {
      ...
      var timerEl  = document.getElementById('story-timer-enabled');
      var mode     = (timerEl && timerEl.checked) ? 'time' : 'round';
      var roundsEl = document.getElementById('input-rounds');
      var timeEl   = document.getElementById('input-turn-time');
      var rounds   = Utils.clamp(parseInt((roundsEl || {}).value || 1), 1, 10);
    ...
    // ── Story: timer-enable toggle ───────────────────────
    var storyTimerToggle = document.getElementById('story-timer-enabled');
    if (storyTimerToggle) {
      storyTimerToggle.addEventListener('change', function() {
        var s = store.get().settings;
        var newMode = storyTimerToggle.checked ? 'time' : 'round';
        var newSettings = Object.assign({}, s, { mode: newMode });
        store.set({ settings: newSettings });
        transport.pushSettings(store.get().roomCode, newSettings);
        self._show('time-setting', newMode === 'time');
      });
    }
    ...
    ['input-rounds','input-turn-time'].forEach(function(id) {
      ...
      if (el) el.addEventListener('change', function() {
        ...
        var ns = Object.assign({}, s, {
          rounds  : Utils.clamp(parseInt(document.getElementById('input-rounds')?.value||1),1,10),
    ...
    // ── Draw Relay: push settings on change ──────────────
    function pushDraw() {
      var s = store.get().settings;
      var dc = {
        writeTime: Utils.clamp(parseInt((document.getElementById('draw-write-time')||{}).value||60),15,180),
        drawTime : Utils.clamp(parseInt((document.getElementById('draw-draw-time')||{}).value||180),20,240),
        guessTime: Utils.clamp(parseInt((document.getElementById('draw-guess-time')||{}).value||45),15,180),
      };
      var ns = Object.assign({}, s, { drawConfig: dc });
      store.set({ settings: ns });
      transport.pushSettings(store.get().roomCode, ns);
    }
    ['draw-write-time','draw-draw-time','draw-guess-time'].forEach(function(id) {
      var el = document.getElementById(id);
      if (el) el.addEventListener('change', pushDraw);
    });

    // ── Draw Relay: start game ───────────────────────────
    this._on('btn-start-draw', 'click', function() {
      var players = store.get().players;
      var activePlayers = Object.values(players).filter(function(p) { return !p.isSpectator; });
      if (activePlayers.length < 3) return self._err('room-error', '傳畫接龍至少需要 3 名玩家');
      var writeTime = Utils.clamp(parseInt((document.getElementById('draw-write-time')||{}).value||60),15,180);
      var drawTime  = Utils.clamp(parseInt((document.getElementById('draw-draw-time')||{}).value||180),20,240);
      var guessTime = Utils.clamp(parseInt((document.getElementById('draw-guess-time')||{}).value||45),15,180);
      var newSettings = Object.assign({}, store.get().settings, { drawConfig: { writeTime: writeTime, drawTime: drawTime, guessTime: guessTime } });
      store.set({ settings: newSettings });
      transport.pushSettings(store.get().roomCode, newSettings);
      drawEngine.startGame({ writeTime: writeTime, drawTime: drawTime, guessTime: guessTime });
      document.querySelectorAll('.screen').forEach(function(el) {
        el.classList.add('hidden'); el.classList.remove('active');
      });
      var scrEl = document.getElementById('screen-draw-game');
      if (scrEl) { scrEl.classList.remove('hidden'); scrEl.classList.add('active'); }
      self._screen = 'draw-game';
      self._drawBoard = null; // force re-mount for fresh game
      self._drawViewChainIdx = null;
      self._renderDraw(store.get());
    });

    // ── Sketch Relay: push settings on change ────────────
    function pushSketch() {
      var s = store.get().settings;
      var timerEl = document.getElementById('sketch-timer-enabled');
      var skc = {
        mode: (timerEl && timerEl.checked) ? 'time' : 'round',
        rounds: Utils.clamp(parseInt((document.getElementById('sketch-rounds')||{}).value||1),1,10),
        turnTime: Utils.clamp(parseInt((document.getElementById('sketch-turn-time')||{}).value||120),30,300),
      };
      var ns = Object.assign({}, s, { sketchConfig: skc });
      store.set({ settings: ns });
      transport.pushSettings(store.get().roomCode, ns);
    }
    var sketchTimerToggle = document.getElementById('sketch-timer-enabled');
    if (sketchTimerToggle) {
      sketchTimerToggle.addEventListener('change', function() {
        self._show('sketch-time-setting', sketchTimerToggle.checked);
        pushSketch();
      });
    }
    ['sketch-rounds','sketch-turn-time'].forEach(function(id) {
      var el = document.getElementById(id);
      if (el) el.addEventListener('change', pushSketch);
    });

    // ── Sketch Relay: start game ──────────────────────────
    this._on('btn-start-sketch', 'click', function() {
      var players = store.get().players;
      var activePlayers = Object.values(players).filter(function(p) { return !p.isSpectator; });
      if (activePlayers.length < CONFIG.MIN_PLAYERS) return self._err('room-error', '至少需要 ' + CONFIG.MIN_PLAYERS + ' 名玩家才能開始');
      var timerEl = document.getElementById('sketch-timer-enabled');
      var mode = (timerEl && timerEl.checked) ? 'time' : 'round';
      var rounds = Utils.clamp(parseInt((document.getElementById('sketch-rounds')||{}).value||1),1,10);
      var turnTime = Utils.clamp(parseInt((document.getElementById('sketch-turn-time')||{}).value||120),30,300);
      var newSettings = Object.assign({}, store.get().settings, { sketchConfig: { mode: mode, rounds: rounds, turnTime: turnTime } });
      store.set({ settings: newSettings });
      transport.pushSettings(store.get().roomCode, newSettings);
      sketchEngine.startGame({ mode: mode, rounds: rounds, turnTime: turnTime });
      document.querySelectorAll('.screen').forEach(function(el) {
        el.classList.add('hidden'); el.classList.remove('active');
      });
      var scrEl = document.getElementById('screen-sketch-game');
      if (scrEl) { scrEl.classList.remove('hidden'); scrEl.classList.add('active'); }
      self._screen = 'sketch-game';
      self._sketchBoard = null;
      self._sketchViewChainIdx = null;
      self._renderSketch(store.get());
    });
  ...
  _bindWWGame() {
    ...
    // Host: advance past the eliminated player's last words
    this._on('btn-ww-last-words-next', 'click', function() {
      wwEngine.sendAction({ type: WW_ACTION.LAST_WORDS_NEXT });
    });
  ...
  _renderWW(s) {
    ...
    // Anyone eliminated in THIS round's vote (including bomber-chain co-victims)
    // must never see their personal death/role-reveal panel — which has a
    // clickable "see anyone's role" table — until the host has fully advanced
    // past last_words. This has to cover 'vote_result' too, not just
    // 'last_words': alive[] flips to false the instant the vote resolves,
    // which is a few seconds *before* last_words actually begins, and without
    // this the death panel would flash up during that gap.
    const rawVoteVoters = g.voteVoters || [];
    const voteVotersArr = Array.isArray(rawVoteVoters) ? rawVoteVoters : Object.values(rawVoteVoters);
    const holdForLastWords = (g.wwPhase === 'vote_result' || g.wwPhase === 'last_words')
      && (myId === g.voteEliminated || voteVotersArr.indexOf(myId) !== -1);

    this._renderWWHeader(g, players, myId);

    // Always hide all game panels first
    const panels = ['role-reveal','night','day-announce','day-discuss','vote','vote-result','last-words','special','end'];
    panels.forEach(p => this._show('ww-panel-' + p, false));

    // Spectator overlay
    this._show('ww-spectator-overlay', !!isSpectator && !isDead);
    // Dead overlay
    this._show('ww-dead-overlay', !!isDead && !isSpectator && !holdForLastWords);
    ...
    if (isDead && !isSpectator) {
      ...
      if (!isSpecialActor && !holdForLastWords) {
      ...
      // Actor / last-words speaker falls through to render the appropriate panel below
    ...
    if (phase === 'last_words')   { this._show('ww-panel-last-words', true);    this._renderWWLastWords(g, players, isHost, myId); }
  ...
  _renderWWSpectator(g, players) {
    ...
    var phaseNames = {
      ...
      last_words:'🎤 遺言時間', special:'⚡ 特殊技能', end:'遊戲結束'
    };
    var phase = g.wwPhase;
  ...
  _renderWWDeadHostBar(g) {
    ...
    } else if (phase === 'last_words' && !g.hunterDecidePending) {
      var btn3 = document.createElement('button');
      btn3.className = 'btn btn-primary btn-sm dead-host-btn';
      btn3.textContent = '➡️ 進入下一階段';
      btn3.onclick = function() { wwEngine.sendAction({ type: WW_ACTION.LAST_WORDS_NEXT }); };
  ...
  _renderWWNight(g, myId, players) {
    ...
    this._setText('ww-night-progress-footer', doneCount + ' / ' + needsAction.length + ' 人完成行動');
  ...
  _renderWWVoteResult(g, players) {
    ...
    this._setText('vote-result-footer-text', isBomber ? '結算中…' : '準備進入遺言時間…');
  ...
  _renderWWLastWords(g, players, isHost, myId) {
    var cont = document.getElementById('ww-last-words-content');
    if (cont) {
      var pid = g.voteEliminated;
      var p   = players[pid] || {};
      var rawVoters = g.voteVoters || [];
      var voters = Array.isArray(rawVoters) ? rawVoters : Object.values(rawVoters);
      var isSpeakerViewing = myId === pid || voters.indexOf(myId) !== -1;
      var hint = isSpeakerViewing
        ? '把握機會留下最後的話，說完後請告知主持人繼續遊戲'
        : Utils.escapeHtml(p.name||'該玩家') + ' 可以趁現在留下最後的話，說完後由主持人繼續遊戲';
      cont.innerHTML =
        '<div class="vr-exile-scene">' +
          '<div class="vr-exile-cross lw-mic-badge">🎤</div>' +
          '<h2 class="lw-title">遺言時間</h2>' +
          '<div class="vr-elim-row">' +
            '<div class="vr-avatar dead-avatar" style="background:' + Utils.avatarColor(p.name||pid) + '">' + (p.name||'?')[0] + '</div>' +
            '<span class="vr-name">' + Utils.escapeHtml(p.name||pid) + '</span>' +
          '</div>' +
          '<p class="vr-no-elim-sub">' + hint + '</p>' +
        '</div>';
    }
    var pending = !!g.hunterDecidePending;
    this._show('btn-ww-last-words-next', isHost && !pending);
    this._show('ww-waiting-last-words', !isHost || pending);
    this._setText('ww-waiting-last-words', pending ? '等待獵人決定是否開槍…' : '等待主持人進入下一階段…');
  }
  ...
  _bindDrawGame() {
    ...
    // write
    var wInp = document.getElementById('draw-write-input');
    this._on('btn-draw-submit-write', 'click', function() {
      var t = (wInp||{}).value || ''; if (!t.trim()) return self.toast('請先輸入題目','warn');
      drawEngine.sendAction({ type: DRAW_ACTION.SUBMIT, content: t.trim() });
    });
    this._on('btn-draw-unsubmit-write', 'click', function() { drawEngine.sendAction({ type: DRAW_ACTION.UNSUBMIT }); });
    if (wInp) wInp.addEventListener('keydown', function(e){ if(e.key==='Enter'&&!e.shiftKey){e.preventDefault(); document.getElementById('btn-draw-submit-write').click();} });

    // draw
    this._on('btn-draw-submit-draw', 'click', function() {
      if (!self._drawBoard) return;
      drawEngine.sendAction({ type: DRAW_ACTION.SUBMIT, content: self._drawBoard.getStrokes() });
    });
    this._on('btn-draw-unsubmit-draw', 'click', function() { drawEngine.sendAction({ type: DRAW_ACTION.UNSUBMIT }); });

    // guess
    var gInp = document.getElementById('draw-guess-input');
    this._on('btn-draw-submit-guess', 'click', function() {
      var t = (gInp||{}).value || ''; if (!t.trim()) return self.toast('請先輸入猜測','warn');
      drawEngine.sendAction({ type: DRAW_ACTION.SUBMIT, content: t.trim() });
    });
    this._on('btn-draw-unsubmit-guess', 'click', function() { drawEngine.sendAction({ type: DRAW_ACTION.UNSUBMIT }); });
    if (gInp) gInp.addEventListener('keydown', function(e){ if(e.key==='Enter'&&!e.shiftKey){e.preventDefault(); document.getElementById('btn-draw-submit-guess').click();} });

    // reveal reactions + chain-switcher browsing (event delegation, same pattern as chaos)
    document.addEventListener('click', function(e) {
      var em = e.target.closest('[data-draw-react]');
      if (em) { drawEngine.sendAction({ type: DRAW_ACTION.REACT, emoji: em.getAttribute('data-emoji') }); return; }
      var tab = e.target.closest('[data-draw-chain]');
      if (tab) {
        self._drawViewChainIdx = parseInt(tab.getAttribute('data-draw-chain'), 10);
        self._renderDraw(store.get());
        return;
      }
    });
    this._on('btn-draw-reveal-next', 'click', function() { drawEngine.sendAction({ type: DRAW_ACTION.REVEAL_NEXT }); });
    this._on('btn-draw-back-lobby',  'click', function() { drawEngine.sendAction({ type: DRAW_ACTION.RETURN_LOBBY }); });
  }

  // ── Draw Relay Rendering ───────────────────────────────

  _renderDraw(s) {
    var g = s.game, players = s.players, myId = s.myId, isHost = s.isHost, isSpectator = s.isSpectator;
    if (!g || g.gameType !== 'draw') return;
    var phase = g.drawPhase;

    // Auto-submit at last second — avoids losing partial input / drawing when time runs out
    if (g.timeLeft === 1 && !isSpectator && (g.submissions||{})[myId] === undefined) {
      if (phase === 'write') {
        var wi = document.getElementById('draw-write-input');
        var wt = wi ? wi.value.trim() : '';
        drawEngine.sendAction({ type: DRAW_ACTION.SUBMIT, content: wt || '（時間到，未輸入）' });
      } else if (phase === 'draw') {
        var strokes = this._drawBoard ? this._drawBoard.getStrokes() : [];
        drawEngine.sendAction({ type: DRAW_ACTION.SUBMIT, content: strokes });
      } else if (phase === 'guess') {
        var gi = document.getElementById('draw-guess-input');
        var gt = gi ? gi.value.trim() : '';
        drawEngine.sendAction({ type: DRAW_ACTION.SUBMIT, content: gt || '（時間到，未輸入）' });
      }
    }

    var specOv = document.getElementById('draw-spectator-overlay');
    var isRevealPhase = phase === 'reveal';
    // Spectators get the simple progress overlay during write/draw/guess, but during
    // the reveal itself they watch the exact same show as everyone else (no spoilers
    // at risk since every chain is already fully resolved by the time reveal starts).
    if (specOv) specOv.classList.toggle('hidden', !isSpectator || isRevealPhase);
    if (isSpectator && !isRevealPhase) { this._renderDrawSpectator(g, players); return; }

    var pn = { write:'✏️ 出題', draw:'🎨 繪圖', guess:'🔍 猜謎', reveal:'🎉 大揭曉' };
    var hdr = document.getElementById('draw-header-info');
    if (hdr) hdr.textContent = phase === 'reveal' ? ('傳畫接龍　' + pn.reveal) : ('第 ' + g.drawRound + ' / ' + g.totalRounds + ' 回合　' + (pn[phase]||''));

    var timerWrap = document.getElementById('draw-timer-wrap'), timerEl = document.getElementById('draw-timer-val');
    var showTimer = ['write','draw','guess'].indexOf(phase) !== -1;
    if (timerWrap) timerWrap.classList.toggle('hidden', !showTimer);
    if (timerEl && showTimer) timerEl.textContent = g.timeLeft || 0;

    ['write','draw','guess','reveal'].forEach(function(p) {
      var el = document.getElementById('draw-panel-' + p); if (el) el.classList.add('hidden');
    });

    if (phase === 'write')  this._renderDrawWrite(g, myId, players);
    if (phase === 'draw')   this._renderDrawDraw(g, myId, players);
    if (phase === 'guess')  this._renderDrawGuess(g, myId, players);
    if (phase === 'reveal') this._renderDrawReveal(g, players, isHost, myId);
  }

  _showDrawPanel(id) { var el = document.getElementById('draw-panel-' + id); if (el) el.classList.remove('hidden'); }

  _drawInputReset(inputId, roundKey) {
    var inp = document.getElementById(inputId);
    if (inp && inp.getAttribute('data-round') !== String(roundKey)) { inp.value = ''; inp.setAttribute('data-round', roundKey); }
  }

  _renderDrawWrite(g, myId, players) {
    this._showDrawPanel('write');
    var submitted = (g.submissions||{})[myId] !== undefined;
    var roundKey  = (g.gameStartTs||0) + '_' + g.drawRound;
    if (!submitted) this._drawInputReset('draw-write-input', roundKey);
    var inp = document.getElementById('draw-write-input'); if (inp) inp.disabled = submitted;
    var btn = document.getElementById('btn-draw-submit-write');
    if (btn) { btn.disabled = submitted; btn.textContent = submitted ? '✓ 已提交，等待其他人…' : '✦ 提交題目'; }
    var unBtn = document.getElementById('btn-draw-unsubmit-write'); if (unBtn) unBtn.classList.toggle('hidden', !submitted);
    var pids = Object.keys(players).filter(function(id){ return !players[id].isSpectator; });
    var prog = document.getElementById('draw-write-progress');
    if (prog) prog.textContent = pids.filter(function(p){ return (g.submissions||{})[p]!==undefined; }).length + ' / ' + pids.length + ' 人已提交';
  }

  _renderDrawDraw(g, myId, players) {
    this._showDrawPanel('draw');
    var submitted = (g.submissions||{})[myId] !== undefined;
    var chainIdx  = (g.assignment||{})[myId];
    var chain     = (g.chains||[])[chainIdx] || { entries: [] };
    var lastEntry = chain.entries[chain.entries.length - 1] || {};
    var promptEl  = document.getElementById('draw-draw-prompt');
    if (promptEl) promptEl.textContent = lastEntry.text || '（無題目）';

    var container = document.getElementById('draw-board-container');
    var roundKey  = (g.gameStartTs||0) + '_' + g.drawRound;
    if (container && container.getAttribute('data-round') !== String(roundKey)) {
      container.setAttribute('data-round', String(roundKey));
      this._drawBoard = new DrawingBoard(container, { readOnly: submitted });
      if (submitted && (g.submissions||{})[myId] !== undefined) this._drawBoard.loadStrokes(g.submissions[myId]);
    } else if (container && this._drawBoard) {
      if (submitted && !this._drawBoard.readOnly) this._drawBoard.setReadOnly(true);
      else if (!submitted && this._drawBoard.readOnly) this._drawBoard.setReadOnly(false);
    }

    var btn = document.getElementById('btn-draw-submit-draw');
    if (btn) btn.classList.toggle('hidden', submitted);
    var unBtn = document.getElementById('btn-draw-unsubmit-draw'); if (unBtn) unBtn.classList.toggle('hidden', !submitted);
    var pids = Object.keys(players).filter(function(id){ return !players[id].isSpectator; });
    var prog = document.getElementById('draw-draw-progress');
    if (prog) prog.textContent = pids.filter(function(p){ return (g.submissions||{})[p]!==undefined; }).length + ' / ' + pids.length + ' 人已提交';
  }

  _renderDrawGuess(g, myId, players) {
    this._showDrawPanel('guess');
    var submitted = (g.submissions||{})[myId] !== undefined;
    var chainIdx  = (g.assignment||{})[myId];
    var chain     = (g.chains||[])[chainIdx] || { entries: [] };
    var lastEntry = chain.entries[chain.entries.length - 1] || {};

    var container = document.getElementById('draw-guess-board-container');
    var roundKey  = (g.gameStartTs||0) + '_' + g.drawRound;
    if (container && container.getAttribute('data-round') !== String(roundKey)) {
      container.setAttribute('data-round', String(roundKey));
      this._drawGuessBoard = new DrawingBoard(container, { readOnly: true });
      this._drawGuessBoard.loadStrokes(lastEntry.strokes || []);
    }

    if (!submitted) this._drawInputReset('draw-guess-input', roundKey);
    var inp = document.getElementById('draw-guess-input'); if (inp) inp.disabled = submitted;
    var btn = document.getElementById('btn-draw-submit-guess');
    if (btn) { btn.disabled = submitted; btn.textContent = submitted ? '✓ 已提交，等待其他人…' : '✦ 提交猜測'; }
    var unBtn = document.getElementById('btn-draw-unsubmit-guess'); if (unBtn) unBtn.classList.toggle('hidden', !submitted);
    var pids = Object.keys(players).filter(function(id){ return !players[id].isSpectator; });
    var prog = document.getElementById('draw-guess-progress');
    if (prog) prog.textContent = pids.filter(function(p){ return (g.submissions||{})[p]!==undefined; }).length + ' / ' + pids.length + ' 人已提交';
  }

  _renderDrawReveal(g, players, isHost, myId) {
    this._showDrawPanel('reveal');
    var chains = g.chains || [];

    // Once fully revealed, players can freely browse any already-revealed chain again
    // (purely local UI state — the underlying game data already holds everything).
    if (g.revealDone) {
      if (this._drawViewChainIdx == null) this._drawViewChainIdx = g.revealChainIndex || 0;
    } else {
      this._drawViewChainIdx = null;
    }

    var ci = g.revealDone ? this._drawViewChainIdx : (g.revealChainIndex || 0);
    var ei = g.revealEntryIndex;
    var label = document.getElementById('draw-reveal-chain-label');
    if (label) label.textContent = g.revealDone
      ? ('第 ' + (ci+1) + ' / ' + chains.length + ' 條鏈　·　可點選下方切換回顧')
      : ('第 ' + (ci+1) + ' / ' + chains.length + ' 條鏈');

    // Chain switcher — only appears once everything has been revealed at least once
    var switcher = document.getElementById('draw-chain-switcher');
    if (switcher) {
      switcher.classList.toggle('hidden', !g.revealDone);
      if (g.revealDone) {
        var switchKey = 'n' + chains.length;
        if (switcher.getAttribute('data-built') !== switchKey) {
          switcher.setAttribute('data-built', switchKey);
          switcher.innerHTML = chains.map(function(c, idx) {
            var starter = players[c.starterPid] || {};
            var color = (typeof Utils.avatarColor === 'function') ? Utils.avatarColor(starter.name || c.starterPid || String(idx)) : '#8a8a8a';
            return '<button type="button" class="draw-chain-tab" data-draw-chain="' + idx + '" style="--tab-color:' + color + '">' +
              '<span class="draw-chain-tab-num">' + (idx+1) + '</span>' +
              '<span class="draw-chain-tab-name">' + Utils.escapeHtml(starter.name || '?') + '</span>' +
            '</button>';
          }).join('');
        }
        switcher.querySelectorAll('.draw-chain-tab').forEach(function(btn) {
          btn.classList.toggle('active', parseInt(btn.getAttribute('data-draw-chain'), 10) === ci);
        });
      }
    }

    var timeline = document.getElementById('draw-reveal-timeline');
    if (timeline) {
      var chain = chains[ci] || { entries: [] };
      var upTo  = g.revealDone ? chain.entries.length - 1 : ei;
      var key   = 'c' + ci + '_e' + upTo + (g.revealDone ? '_done' : '');
      if (timeline.getAttribute('data-key') !== key) {
        timeline.setAttribute('data-key', key);
        // Clean up any DrawingBoard instances from the previous card before wiping the DOM
        if (this._drawRevealBoards) { this._drawRevealBoards.forEach(function(b){ try { b.destroy(); } catch(_){} }); }
        this._drawRevealBoards = [];
        var html = '';
        for (var i = 0; i <= upTo && i < chain.entries.length; i++) {
          var entry = chain.entries[i];
          var author = players[entry.authorPid] || {};
          var typeLabel = entry.type === 'write' ? '📝 出題' : entry.type === 'draw' ? '🎨 繪圖' : '🔍 猜測';
          html += '<div class="draw-timeline-item">' +
            '<div class="draw-timeline-meta"><span class="draw-timeline-type">' + typeLabel + '</span><span class="draw-timeline-author">' + Utils.escapeHtml(author.name||'???') + '</span></div>';
          if (entry.type === 'draw') {
            html += '<div class="draw-timeline-canvas-mount" data-strokes-idx="' + i + '"></div>';
          } else {
            html += '<div class="draw-timeline-text">' + Utils.escapeHtml(entry.text || '') + '</div>';
          }
          html += '</div>';
          if (i < upTo) html += '<div class="draw-timeline-arrow">↓</div>';
        }
        timeline.innerHTML = html || '<div class="chaos-progress-text">準備揭示…</div>';
        var self = this;
        var mounts = timeline.querySelectorAll('.draw-timeline-canvas-mount');
        mounts.forEach(function(m) {
          var idx = parseInt(m.getAttribute('data-strokes-idx'), 10);
          var e = chain.entries[idx];
          var b = new DrawingBoard(m, { readOnly: true });
          b.loadStrokes((e||{}).strokes || []);
          self._drawRevealBoards.push(b);
        });
        timeline.scrollTop = timeline.scrollHeight;
      }
    }

    var nextBtn = document.getElementById('btn-draw-reveal-next');
    var watchTxt = document.getElementById('draw-reveal-watch');
    var backBtn = document.getElementById('btn-draw-back-lobby');
    if (nextBtn)  nextBtn.classList.toggle('hidden', !isHost || !!g.revealDone);
    if (watchTxt) {
      if (isHost) {
        watchTxt.classList.add('hidden');
      } else if (!g.revealDone) {
        watchTxt.textContent = '等待主持人揭示…';
        watchTxt.classList.remove('hidden');
      } else {
        watchTxt.textContent = '🎉 播放完畢，等待主持人返回大廳…';
        watchTxt.classList.remove('hidden');
      }
    }
    if (backBtn)  backBtn.classList.toggle('hidden', !isHost || !g.revealDone);

    // Reactions (row + floating stage) — same visual pattern as 規則混亂
    var reactions = g.reactions || {};
    var reactBtns = document.getElementById('draw-react-btns');
    if (reactBtns) {
      var reactEmojis = ['😂','🔥','💀','👏','😮','🤣'];
      var myReact = reactions[myId] || '';
      var curMark = reactBtns.getAttribute('data-react');
      if (curMark === null || curMark !== myReact) {
        reactBtns.setAttribute('data-react', myReact);
        reactBtns.innerHTML = reactEmojis.map(function(em) {
          return '<button class="chaos-react-btn' + (myReact===em?' reacting':'') + '" data-draw-react="1" data-emoji="' + em + '">' + em + '</button>';
        }).join('');
      }
    }
    var stage = document.getElementById('draw-react-stage');
    if (stage) {
      var existing = {};
      stage.querySelectorAll('[data-react-pid]').forEach(function(el) { existing[el.getAttribute('data-react-pid')] = el; });
      Object.keys(existing).forEach(function(pid) { if (!reactions[pid]) existing[pid].remove(); });
      Object.entries(reactions).forEach(function(pair, idx) {
        var pid2 = pair[0], emoji = pair[1];
        if (!existing[pid2]) {
          var span = document.createElement('span');
  ...
  _renderDrawSpectator(g, players) {
    var cont = document.getElementById('draw-spectator-content');
    if (!cont) return;
    var phase = g.drawPhase;
    var pn = { write:'✏️ 出題中', draw:'🎨 繪圖中', guess:'🔍 猜謎中', reveal:'🎉 揭曉中' };
    var pids = Object.keys(players).filter(function(id){ return !players[id].isSpectator; });
    var doneCount = pids.filter(function(p){ return (g.submissions||{})[p]!==undefined; }).length;
    var html = '<div class="spec-lobby-row"><span>🎨 傳畫接龍　' + (pn[phase]||'') + '</span></div>';
    if (phase !== 'reveal') {
      html += '<div class="spec-lobby-row"><span class="slr-label">第 ' + g.drawRound + ' / ' + g.totalRounds + ' 回合</span><span>' + doneCount + ' / ' + pids.length + ' 人已提交</span></div>';
    } else {
      html += '<div class="spec-lobby-row"><span class="slr-label">進度</span><span>第 ' + ((g.revealChainIndex||0)+1) + ' / ' + (g.chains||[]).length + ' 條鏈</span></div>';
    }
    cont.innerHTML = html;
  }

  // ── Sketch Relay Binding & Rendering ───────────────────

  _bindSketchGame() {
    var self = this;

    this._on('btn-sketch-lock', 'click', function() {
      if (!self._sketchBoard) return;
      if (self._sketchBoard.isBlank()) return self.toast('至少畫一筆再鎖定喔', 'warn');
      sketchEngine.sendAction({ type: SKETCH_ACTION.LOCK, content: self._sketchBoard.getStrokes() });
    });
    this._on('btn-sketch-unlock', 'click', function() { sketchEngine.sendAction({ type: SKETCH_ACTION.UNLOCK }); });

    document.addEventListener('click', function(e) {
      var em = e.target.closest('[data-sketch-react]');
      if (em) { sketchEngine.sendAction({ type: SKETCH_ACTION.REACT, emoji: em.getAttribute('data-emoji') }); return; }
      var tab = e.target.closest('[data-sketch-chain]');
      if (tab) {
        self._sketchViewChainIdx = parseInt(tab.getAttribute('data-sketch-chain'), 10);
        self._renderSketch(store.get());
        return;
      }
    });
    this._on('btn-sketch-reveal-next', 'click', function() { sketchEngine.sendAction({ type: SKETCH_ACTION.REVEAL_NEXT }); });
    this._on('btn-sketch-back-lobby',  'click', function() { sketchEngine.sendAction({ type: SKETCH_ACTION.RETURN_LOBBY }); });
  }
  ...
  _renderSketch(s) {
    ...
    // Auto-lock at the last second in timed mode — avoids losing whatever was drawn
    if (g.mode === 'time' && phase === 'drawing' && g.timeLeft === 1 && !isSpectator && !(g.locked||{})[myId]) {
      var strokes = this._sketchBoard ? this._sketchBoard.getStrokes() : { base: null, strokes: [] };
      sketchEngine.sendAction({ type: SKETCH_ACTION.LOCK, content: strokes });
    }

    var specOv = document.getElementById('sketch-spectator-overlay');
    var isRevealPhase = phase === 'reveal';
    if (specOv) specOv.classList.toggle('hidden', !isSpectator || isRevealPhase);
    if (isSpectator && !isRevealPhase) { this._renderSketchSpectator(g, players); return; }

    var hdr = document.getElementById('sketch-header-info');
    if (hdr) hdr.textContent = phase === 'reveal' ? '繪畫接龍　🎉 大揭曉' : ('第 ' + g.currentTurn + ' / ' + g.totalTurns + ' 輪　🖼️ 接力中');

    var timerWrap = document.getElementById('sketch-timer-wrap'), timerEl = document.getElementById('sketch-timer-val');
    var showTimer = phase === 'drawing' && g.mode === 'time';
    if (timerWrap) timerWrap.classList.toggle('hidden', !showTimer);
    if (timerEl && showTimer) timerEl.textContent = g.timeLeft || 0;

    ['draw','reveal'].forEach(function(p) {
      var el = document.getElementById('sketch-panel-' + p); if (el) el.classList.add('hidden');
    });

    if (phase === 'drawing') this._renderSketchDraw(g, myId, players);
    if (phase === 'reveal')  this._renderSketchReveal(g, players, isHost, myId);
  }

  _renderSketchDraw(g, myId, players) {
    var el = document.getElementById('sketch-panel-draw'); if (el) el.classList.remove('hidden');
    var locked = !!(g.locked||{})[myId];
    var chainIdx = (g.assignments||{})[myId];
    var chain = (g.canvases||[])[chainIdx] || [];
    var isFresh = chain.length === 0;

    var titleEl = document.getElementById('sketch-draw-title');
    var hintEl  = document.getElementById('sketch-draw-hint');
    if (titleEl) titleEl.textContent = isFresh ? '🖼️ 從空白開始創作！' : '🖼️ 接續這幅畫！';
    if (hintEl)  hintEl.textContent  = isFresh ? '畫下你想畫的任何東西，開啟這條畫布的接力' : '在既有的畫面上繼續添加，畫完後傳給下一位';

    var container = document.getElementById('sketch-board-container');
    var roundKey = (g.gameStartTs||0) + '_' + g.currentTurn;
    if (container && container.getAttribute('data-round') !== String(roundKey)) {
      container.setAttribute('data-round', String(roundKey));
      this._sketchBoard = new DrawingBoard(container, { readOnly: locked, addOnly: true });
      var inherited = isFresh ? null : chain[chain.length - 1].snapshot;
      if (inherited) this._sketchBoard.loadStrokes(inherited);
      if (locked && (g.submissions||{})[myId]) this._sketchBoard.loadStrokes(g.submissions[myId]);
    } else if (container && this._sketchBoard) {
      if (locked && !this._sketchBoard.readOnly) this._sketchBoard.setReadOnly(true);
      else if (!locked && this._sketchBoard.readOnly) this._sketchBoard.setReadOnly(false);
    }

    var lockBtn = document.getElementById('btn-sketch-lock');
    if (lockBtn) lockBtn.classList.toggle('hidden', locked);
    var unlockBtn = document.getElementById('btn-sketch-unlock');
    if (unlockBtn) unlockBtn.classList.toggle('hidden', !locked);

    var pids = Object.keys(players).filter(function(id){ return !players[id].isSpectator; });
    var prog = document.getElementById('sketch-draw-progress');
    if (prog) prog.textContent = pids.filter(function(p){ return (g.locked||{})[p]; }).length + ' / ' + pids.length + ' 人已鎖定';
  }

  _renderSketchReveal(g, players, isHost, myId) {
    var el = document.getElementById('sketch-panel-reveal'); if (el) el.classList.remove('hidden');
    var canvases = g.canvases || [];

    if (g.revealDone) {
      if (this._sketchViewChainIdx == null) this._sketchViewChainIdx = g.revealChainIndex || 0;
    } else {
      this._sketchViewChainIdx = null;
    }
    var ci = g.revealDone ? this._sketchViewChainIdx : (g.revealChainIndex || 0);
    var ei = g.revealEntryIndex;

    var label = document.getElementById('sketch-reveal-chain-label');
    if (label) label.textContent = g.revealDone
      ? ('第 ' + (ci+1) + ' / ' + canvases.length + ' 條畫布　·　可點選下方切換回顧')
      : ('第 ' + (ci+1) + ' / ' + canvases.length + ' 條畫布');

    var switcher = document.getElementById('sketch-chain-switcher');
    if (switcher) {
      switcher.classList.toggle('hidden', !g.revealDone);
      if (g.revealDone) {
        var switchKey = 'n' + canvases.length;
        if (switcher.getAttribute('data-built') !== switchKey) {
          switcher.setAttribute('data-built', switchKey);
          switcher.innerHTML = canvases.map(function(chain, idx) {
            var starter = players[(chain[0]||{}).authorId] || {};
            var color = (typeof Utils.avatarColor === 'function') ? Utils.avatarColor(starter.name || String(idx)) : '#8a8a8a';
            return '<button type="button" class="draw-chain-tab" data-sketch-chain="' + idx + '" style="--tab-color:' + color + '">' +
              '<span class="draw-chain-tab-num">' + (idx+1) + '</span>' +
              '<span class="draw-chain-tab-name">' + Utils.escapeHtml(starter.name || '?') + '</span>' +
            '</button>';
          }).join('');
        ...
        switcher.querySelectorAll('.draw-chain-tab').forEach(function(btn) {
          btn.classList.toggle('active', parseInt(btn.getAttribute('data-sketch-chain'), 10) === ci);
        });
      }
    }

    var timeline = document.getElementById('sketch-reveal-timeline');
    if (timeline) {
      var chain = canvases[ci] || [];
      var upTo = g.revealDone ? chain.length - 1 : ei;
      var key = 'c' + ci + '_e' + upTo + (g.revealDone ? '_done' : '');
      if (timeline.getAttribute('data-key') !== key) {
        timeline.setAttribute('data-key', key);
        if (this._sketchRevealBoards) { this._sketchRevealBoards.forEach(function(b){ try { b.destroy(); } catch(_){} }); }
        this._sketchRevealBoards = [];
        var html = '';
        for (var i = 0; i <= upTo && i < chain.length; i++) {
          var turn = chain[i];
          html += '<div class="draw-timeline-item">' +
            '<div class="draw-timeline-meta"><span class="draw-timeline-type">🖼️ 第 ' + (i+1) + ' 筆</span><span class="draw-timeline-author">' + Utils.escapeHtml((turn||{}).authorName || '???') + '</span></div>' +
            '<div class="draw-timeline-canvas-mount" data-turn-idx="' + i + '"></div>' +
          '</div>';
          if (i < upTo) html += '<div class="draw-timeline-arrow">↓</div>';
        }
        timeline.innerHTML = html || '<div class="chaos-progress-text">準備揭示…</div>';
        var self = this;
        timeline.querySelectorAll('.draw-timeline-canvas-mount').forEach(function(m) {
          var idx = parseInt(m.getAttribute('data-turn-idx'), 10);
          var t = chain[idx];
          var b = new DrawingBoard(m, { readOnly: true });
          b.loadStrokes((t||{}).snapshot || []);
          self._sketchRevealBoards.push(b);
        });
        timeline.scrollTop = timeline.scrollHeight;
      }
    }

    var nextBtn = document.getElementById('btn-sketch-reveal-next');
    var watchTxt = document.getElementById('sketch-reveal-watch');
    var backBtn = document.getElementById('btn-sketch-back-lobby');
    if (nextBtn) nextBtn.classList.toggle('hidden', !isHost || !!g.revealDone);
    if (watchTxt) {
      if (isHost) watchTxt.classList.add('hidden');
      else if (!g.revealDone) { watchTxt.textContent = '等待主持人揭示…'; watchTxt.classList.remove('hidden'); }
      else { watchTxt.textContent = '🎉 播放完畢，等待主持人返回大廳…'; watchTxt.classList.remove('hidden'); }
    }
    if (backBtn) backBtn.classList.toggle('hidden', !isHost || !g.revealDone);

    var reactions = g.reactions || {};
    var reactBtns = document.getElementById('sketch-react-btns');
    if (reactBtns) {
      var reactEmojis = ['😂','🔥','💀','👏','😮','🤣'];
      var myReact = reactions[myId] || '';
      var curMark = reactBtns.getAttribute('data-react');
      if (curMark === null || curMark !== myReact) {
        reactBtns.setAttribute('data-react', myReact);
        reactBtns.innerHTML = reactEmojis.map(function(em) {
          return '<button class="chaos-react-btn' + (myReact===em?' reacting':'') + '" data-sketch-react="1" data-emoji="' + em + '">' + em + '</button>';
        }).join('');
      }
    }
    var stage = document.getElementById('sketch-react-stage');
    if (stage) {
      var existing = {};
      stage.querySelectorAll('[data-react-pid]').forEach(function(el) { existing[el.getAttribute('data-react-pid')] = el; });
      Object.keys(existing).forEach(function(pid) { if (!reactions[pid]) existing[pid].remove(); });
  ...
  _renderSketchSpectator(g, players) {
    var cont = document.getElementById('sketch-spectator-content');
    if (!cont) return;
    var pids = Object.keys(players).filter(function(id){ return !players[id].isSpectator; });
    var doneCount = pids.filter(function(p){ return (g.locked||{})[p]; }).length;
    var html = '<div class="spec-lobby-row"><span>🖼️ 繪畫接龍　接力中</span></div>' +
      '<div class="spec-lobby-row"><span class="slr-label">第 ' + g.currentTurn + ' / ' + g.totalTurns + ' 輪</span><span>' + doneCount + ' / ' + pids.length + ' 人已鎖定</span></div>';
    cont.innerHTML = html;
  }

  // ── Invite / Share ────────────────────────────────────

  _buildInviteUrl(code) {
    try {
      var s = store.get();
      var hostName = '';
      if (s.players && s.hostId && s.players[s.hostId]) hostName = s.players[s.hostId].name || '';
  ...
  _shareOrCopyInvite(code) {
    var self = this;
    if (!code) return;
    var url = this._buildInviteUrl(code);
    if (navigator.share) {
      navigator.share({ url: url }).catch(function() { /* user cancelled — no-op */ });
      return;
    }
    if (navigator.clipboard) {
      navigator.clipboard.writeText(url).then(function() {
        self.toast('邀請連結已複製！', 'success');
      }).catch(function() {
        self.toast('連結：' + url, 'info');
      });
    } else {
      self.toast('連結：' + url, 'info');
    }
  ...
  _openQrModal(code) {
    ...
    var codeEl = document.getElementById('qr-room-code');
    ...
    if (typeof qrcode !== 'function') {
      wrap.innerHTML = '<div class="qr-loading">QR Code 功能無法使用，請改用下方分享連結</div>';
      return;
    }

    try {
      // Try increasing QR "type number" (data capacity) until the URL fits —
      // avoids ever hard-coding a size that overflows for longer names/URLs.
      var qr = null;
      for (var tn = 2; tn <= 40; tn++) {
        try {
          var candidate = qrcode(tn, 'M');
          candidate.addData(url);
          candidate.make();
          qr = candidate;
          break;
        } catch (eInner) { /* too small — try next size */ }
      }
      if (!qr) throw new Error('QR data too long');

      var count = qr.getModuleCount();
      var pixelSize = 200;
      var cellSize = Math.max(1, Math.floor(pixelSize / count));
      ...
      wrap.innerHTML = '<div class="qr-loading">QR Code 產生失敗，請改用下方分享連結</div>';
    }
  }
  ...
  _bindShareInvite() {
    ...
    this._on('btn-share-room', 'click', function() {
      var code = ((document.getElementById('display-room-code') || {}).textContent || '').trim();
      self._shareOrCopyInvite(code);
    });

    this._on('btn-show-qr', 'click', function() {
      var code = ((document.getElementById('display-room-code') || {}).textContent || '').trim();
      self._openQrModal(code);
    });

    this._on('btn-qr-share', 'click', function() {
      var code = ((document.getElementById('qr-room-code') || {}).textContent || '').trim();
      self._shareOrCopyInvite(code);
    });

    this._on('btn-qr-close', 'click', function() {
  ...
  _checkDeepLink() {
    try {
      ...
      var roomParam = (params.get('room') || '').toUpperCase().trim();
      if (!roomParam) return;
      var hostParam = (params.get('host') || '').trim();

      if (hostParam) {
        // Dedicated invite landing screen — shows who invited you
        var hostEl = document.getElementById('invite-host-name');
        var codeEl = document.getElementById('invite-room-code');
        if (hostEl) hostEl.textContent = hostParam;
        if (codeEl) codeEl.textContent = roomParam;
        this._inviteRoomCode = roomParam;
        this.show('invite');
        var self = this;
        setTimeout(function() {
          var ni = document.getElementById('invite-input-name');
          if (ni) ni.focus();
        }, 300);
        return;
      }

      // No host name known — fall back to pre-filling the regular home screen
      ...
      this.toast('偵測到邀請連結，房間代碼已自動填入！', 'info');
  ...
  _bindInviteScreen() {
    ...
    var go = async function() {
      if (_busy) return;
      var name = (document.getElementById('invite-input-name') || {}).value;
      name = name ? name.trim() : '';
      if (!name) return self._err('invite-error', '請先輸入暱稱');
      var code = self._inviteRoomCode || '';
      if (!code) return self._err('invite-error', '找不到房間代碼');
      _busy = true;
      self._setInviteBusy(true);
      try {
        await roomManager.joinRoom(code, name);
      } catch (e) {
        self._err('invite-error', '加入失敗：' + e.message);
      } finally {
        _busy = false;
        self._setInviteBusy(false);
      }
    };

    this._on('btn-invite-join', 'click', go);
    this._on('invite-input-name', 'keydown', function(e) { if (e.key === 'Enter') go(); });
    this._on('btn-invite-not-me', 'click', function() {
      // Strip the query string so re-entering the invite screen doesn't loop, then go home
      try {
        var url = new URL(window.location.href);
        url.search = ''; url.hash = '';
  ...
  _setInviteBusy(busy) {
    var joinBtn  = document.getElementById('btn-invite-join');
    var nameInp  = document.getElementById('invite-input-name');
    var loading  = document.getElementById('invite-loading-bar');
    if (joinBtn) { joinBtn.disabled = busy; joinBtn.classList.toggle('btn-loading', busy); }
    if (nameInp) nameInp.disabled = busy;
